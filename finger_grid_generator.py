#THIS SCRIPT GENERATES THE FINGER GRIDS FOR INPUT
...
output_grids =  "-p" in opts #should program out finger grids as it generates them or not?
...
while True:
    ...
    if(not key.is_pressed('w') and not make_grid):
        can_generate = True

    if key.is_pressed('w') and can_generate:
        make_grid = True
        can_generate = False
